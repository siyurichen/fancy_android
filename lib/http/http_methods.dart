...
class HttpMethods {
  ...
  HttpMethods() {
    _dio.interceptors
      ...
      ..add(InterceptorsWrapper(onRequest: (RequestOptions options) async {
        var cookies = await SharedPreferencesUtil.get(
            SharedPreferencesUtil.SHARE_PREF_COOKIE);
        ...
        if (null != cookies && cookies.isNotEmpty) {
          SharedPreferencesUtil.put(
              SharedPreferencesUtil.SHARE_PREF_COOKIE, cookies.toString());
        }
      }));
  }
  ...
  Future<banner.HomeBannerModel> getBanner() async {
    var response = await _sendRequest(POST, Api.HOME_BANNER_URL, null);
    try {
  ...
  Future<ProjectCategoryModel> getProjectCategory() async {
    var response = await _sendRequest(GET, Api.PROJECT_CATEGORY_URL, null);
    try {
  ...
  Future<system.KnowledgeSystemModel> getKnowledgeSystem() async {
    var response;
    try {
      response = await _sendRequest(GET, Api.KNOWLEDGE_SYSTEM_URL, null);
  ...
  Future<hotSearchKey.HotSearchKeyModel> getHotSearchKey() async {
    var response = await _sendRequest(GET, Api.HOT_SEARCH_KEY_URL, null);
    try {
  ...
  Future<commonUseWebsiteModel.CommonUseWebsiteModel>
    ...
    var response = await _sendRequest(GET, Api.COMMON_USE_WEBSITE_URL, null);
    try {
  ...
  Future<article.Data> searchArticle(String url, String keyName) async {
    var data = {'k': keyName};
    var response = await _sendRequest(POST, url, data);
    try {
  ...
  Future<article.Data> getArticle(String url) async {
    var response;
    try {
      response = await _sendRequest(GET, url, null);
      return article.Data.fromJson(response['data']);
    } catch (error) {
      throw response['errorMsg'];
    }
  ...
  Future<WeChatArticleCategory> getWeChatArticleCategory() async {
    var response =
        await _sendRequest(GET, Api.WE_CHAT_ARTICLE_CATEGORY_URL, null);
    try {
  ...
  Future login(String userName, String passWord) async {
    ...
    var response = await _sendRequest(POST, Api.LOGIN_URL, map);
    try {
      return response;
    } catch (error) {
      throw response['errorMsg'];
  ...
  Future doOptionRequest({String url, Map<String, dynamic> params}) async {
    var response = await _sendRequest(POST, url, params);
    try {
